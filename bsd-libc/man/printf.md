...
\*newfmt(const char \*fmt, \...) { char \*p; va\_list ap; if ((p =
malloc(128)) == NULL) return (NULL); va\_start(ap, fmt); (void)
vsnprintf(p, 128, fmt, ap); va\_end(ap); return (p); }
...
buffer overflow attack. Because
...
void foo(const char \*arbitrary\_string, const char \*and\_another) {
char onstack\[8\];

\#ifdef BAD /\* \* This first sprintf is bad behavior. Do not use
sprintf! \*/ sprintf(onstack, \"%s, %s\", arbitrary\_string,
and\_another); \#else /\* \* The following two lines demonstrate better
use of \* snprintf(). \*/ snprintf(onstack, sizeof(onstack), \"%s, %s\",
arbitrary\_string, and\_another); \#endif }
