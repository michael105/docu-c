...
The *asprintf*() and *vasprintf*() functions set /*ret/ to be a pointer
...
*asprintf*() and *vasprintf*() will return −1 and set /ret/ to be a NULL
pointer.
...
The *sprintf*() and *vsprintf*() functions effectively assume a /size/
of INT_MAX + 1.
...
equivalent in size to a /size_t/.
...
− ]0x/h/./hhhp/[\\
± ]/d/, where the number of digits after the hexadecimal-point character
...
from OpenBSD 2.3 by Todd C. Miller </Todd.Miller@courtesan.com/>. The
